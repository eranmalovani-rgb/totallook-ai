...
/* ──────────────────────── Find matching item for an improvement ──────────────────────── */
...
export default function FixMyLookModal({ reviewId, analysis, trigger }: FixMyLookModalProps) {
  ...
  useEffect(() => {
    if (savedResultQuery.data && open && step === "select" && !result && !skipRestore) {
      const saved = savedResultQuery.data;
      setResult({
        ...
        itemsFixed: saved.itemsFixed || [],
        shoppingLinks: (saved.shoppingLinks || []) as unknown as ShoppingLink[],
      });
      setStep("result");
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `fix-my-look-${reviewId}.png`;
