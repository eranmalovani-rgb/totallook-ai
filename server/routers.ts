...
export function buildRecommendationsPromptFromCore(
  ...
  const occasionLine = occasion && occasion !== 'general'
    ? (isHebrew
     ...
     : (isHebrew ? "אין אירוע יעד מחייב. חובה: זהה את ההקשר/סביבה מתוך נתוני שלב 1 (סיכום, פריטים, רקע) והתאם את כל ההמלצות להקשר המזוהה. אם הלוק אלגנטי והרקע חשוך — אל תמליץ על בית קפה! אם הבגדים ספורטיביים (טייץ, נעלי ספורט, לגינס) — המלץ רק על שדרוגי ספורט/אימון (נעלי ריצה, בגדי אימון, סטים ספורטיביים). אסור להמליץ על ג'ינס, שמלות או פריטים פורמליים!" : "No strict target occasion. IMPORTANT: Detect the likely occasion/context from Stage 1 data (summary, items, background) and tailor ALL recommendations to match. If the look is elegant and the setting is dark/formal \u2014 do NOT recommend cafe-style upgrades! If the clothing is sportswear/athletic (leggings, sneakers, tank tops, sports bra) \u2014 recommend ONLY sport/athletic upgrades (running shoes, moisture-wicking fabrics, matching athletic sets). NEVER suggest jeans, dresses, or formal items for workout outfits!");
