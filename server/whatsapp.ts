...
async function fetchWithRetry(url: string, init: RequestInit, label: string): Promise<globalThis.Response> {
...
async function handleIncomingMessage(message: MetaWhatsAppMessage, profileName: string) {
  ...
  if (!hasImage) {
    await sendWhatsAppMessage(
      ...
      `⚡ יש לך *3 ניתוחים חינמיים* — ${g.send} תמונה ו${g.try_}!\n\n` +
      `💎 ${g.register} (10 שניות עם Google) ו${g.receive} הכל:\n` +
      `✓ ניתוחים ללא הגבלה\n` +
      `✓ המלצות מותאמות לסגנון, תקציב ומיקום שלך\n` +
      `✓ ארון בגדים חכם שזוכר את הכל\n` +
      `✓ השראה ממשפיענים שמתאימים לך\n\n` +
      `👉 ${SITE_URL}`,
...
async function downloadMetaMedia(mediaId: string): Promise<Buffer> {
  ...
  return Buffer.from(await (fileResponse as globalThis.Response).arrayBuffer());
}
...
async function sendWhatsAppWelcome(
  ...
  const message = isRegistered
    ...
    : `${greeting} \u{1F44B}\n\n` +
      `זה TotalLook.ai \u2014 הסטייליסט הדיגיטלי שלך \u2728\n\n` +
      `${g.happy} שהשארת את המספר! עכשיו ${g.youCan} לשלוח לי תמונה של הלוק שלך ולקבל ניתוח אופנתי \u{1F4F8}\n\n` +
      `\u26A1 יש לך *3 ניתוחים חינמיים* \u2014 ${g.send} תמונה ו${g.try_}!\n\n` +
      `\u{1F48E} ${g.register} (10 שניות עם Google) ו${g.receive} הכל:\n` +
      `\u2713 ניתוחים ללא הגבלה\n` +
      `\u2713 המלצות מותאמות לסגנון, תקציב ומיקום שלך\n` +
      `\u2713 ארון בגדים חכם שזוכר את הכל\n` +
      `\u2713 השראה ממשפיענים שמתאימים לך\n\n` +
